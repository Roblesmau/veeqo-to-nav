...
- Export Available to Take: pivot of `public.inventory_available` — one row per SKU, one column per `location_code` (alphabetical) + `Total`, enriched with every `mau_main_items` column. All zones, always fresh, filename `available_to_take_YYYY-MM-DD_HHMMSS.xlsx`. Its `MD_COLS` array duplicates Export Items MD's column list — **add new `mau_main_items` columns to both** (they're independent on purpose).
...
- `resolveMapping(storeId, ecom, rawSku)` is **smart-fallback**, not strict: it tries the store's configured column (ecom/sku/upc) first, then walks the other two indexes, probing the raw value as both the ecom and sku/upc key. Returns null only when nothing matches anywhere. (Was strict per-store until 62fe789 — a misconfigured store dropdown no longer silently empties SKU/UPC.)
- Veeqo line items expose both `sku_code` and `upc_code`. The same purchase order can return as **duplicate records** (same `number`, different `delivery_method`); deduped by `number` keeping the latest `updated_at` after the fetch loop (5c60ff9).
...
- **Diagnose "missing SKU/UPC" in the browser console first, before editing `resolveMapping`.** Have the user paste `!!MAPPING_BY_SKU['key']`, `Object.keys(MAPPING_BY_SKU).length`, and `ITEMS_SOURCE` into DevTools. A populated index + a `false` lookup = the row genuinely isn't in `mau_main_items` (source-data gap, often a Nav-side SQL view filtering a category like `ACC`) — no code change will surface it. Verify with a Supabase SQL Editor query (`where item_no=… or no_2=… or upc in (…)`) before assuming a lookup bug.
- **Dump raw Veeqo orders with `copy(JSON.stringify(ORDERS.filter(o=>String(o.number)==='<num>'),null,2))` in the console.** Confirms the real line-item shape (both `sku_code` and `upc_code` are present) — faster than guessing at the JSON structure.
