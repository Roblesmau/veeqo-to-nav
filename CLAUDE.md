...
- The Supabase anon key embedded in `index.html` is intentional (single-workspace model, RLS-open). If it ever stops working, **decode the JWT's `ref` claim** before debugging policies — the most likely cause is that `SB_URL` and the anon key point at different projects.
